...
TEST_CASE("pct_decode_test")
{
  ...
  {
    auto const input = std::string_view("www.%c5%bc%c3%b3%c5%82%c4%87.pl");

    auto bytes = std::array<char, 256>{0};
    auto ec    = boost::system::error_code();

    auto const decoded_view = std::string_view(
      bytes.data(), furi::uri::pct_decode(input, bytes.begin(), ec) - bytes.begin());
  ...
  {
    auto const input = std::string_view("hello%20world!%0a");
  ...
  {
    auto const input = std::string_view("%22%23%2f%3c%3e%3f%40%5b%5c%5d%5e%60%7b%7c%7d");
  ...
  {
    auto const input = std::string_view("/%22%23%3c%3e%3f@%5b%5c%5d%5e%60%7b%7c%7d:::");
  ...
  SECTION("should recognize malformed input and simply stop parsing")
  {
    ...
    auto const invalid_host = std::string_view("www.goo%%%%gle.com");

    auto pos = invalid_host.begin();
    x3::parse(pos, invalid_host.end(), furi::uri::host);
    ...
    auto decoded_view = std::string_view(
      bytes.data(), furi::uri::pct_decode(invalid_host, bytes.begin(), ec) - bytes.begin());

    CHECK(decoded_view == "www.goo");
    CHECK(ec == furi::error::unexpected_pct);

    bytes.fill(0);

    auto out = bytes.begin();
    CHECK_THROWS(furi::uri::pct_decode(invalid_host, out));

    decoded_view = std::string_view(bytes.data(), out - bytes.begin());
    CHECK(decoded_view == "www.goo");
  }

  SECTION("should handle an incomplete pct-encoded byte")
  {
    auto const input = std::string_view("%2");
  ...
  SECTION("should handle an invalid pct-encoded byte")
  {
    ...
    input = std::string_view("%1g");

    decoded_view = std::string_view(bytes.data(), furi::uri::pct_decode(input, bytes.begin(), ec) -
                                                    bytes.begin());
  ...
  {
    auto const input = std::string_view("\x00\xff\x8f\x01");
